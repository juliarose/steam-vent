...
use crate::{
    ...
    proto::{
        ...
        steammessages_clientserver_2::CMsgGCClient,
...
impl ClientFromGCMessage {
    ...
    pub fn from_message(msg: RawNetMessage) -> Result<Self, NetworkError> {
        ...
        let appid = msg.appid();
        let msgtype = msg.msgtype() as i32 & (!PROTO_MASK) as i32;
        let payload = msg.payload();
        let is_proto = (msg.msgtype() as i32 & PROTO_MASK as i32) != 0;
        ...
        let (target_job_id, payload) = if is_proto {
            ...
            let target_job_id = header.jobid_target();
